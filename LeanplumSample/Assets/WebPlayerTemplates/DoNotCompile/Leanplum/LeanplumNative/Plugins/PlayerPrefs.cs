 ...
 * Public Domain
 *
 * To the extent possible under law, PreviewLabs has waived all copyright and related or
 * neighboring rights to this document. This work is published from: Belgium.
 *
...
namespace LeanplumSDK.Prefs {
    ...
    private static void Serialize() {
      ...
      while ( myEnumerator.MoveNext() ) {
        if (myEnumerator != null && myEnumerator.Key != null && myEnumerator.Value != null) {
          ...
          serializedOutput += EscapeNonSeperators(myEnumerator.Key.ToString()) + " "
          + KEY_VALUE_SEPERATOR + " " + EscapeNonSeperators(myEnumerator.Value.ToString()) + " "
          + KEY_VALUE_SEPERATOR + " " + myEnumerator.Value.GetType();
        }
      }
    }
    ...
    private static void Deserialize() {
      ...
      foreach(string parameter in parameters) {
        string[] parameterContent = parameter.Split(new string[]{" " + KEY_VALUE_SEPERATOR + " "},
          StringSplitOptions.None);

        if (parameterContent.Length >= 3) {
          string key = DeEscapeNonSeperators(parameterContent[0]);
          playerPrefsHashtable[key] = GetTypeValue(parameterContent[2],
            DeEscapeNonSeperators(parameterContent[1]));
        }
        if (parameterContent.Length != 3) {
          Debug.LogWarning("PlayerPrefs::Deserialize() parameterContent has " +
            parameterContent.Length + " elements");
        }
      }
    }
